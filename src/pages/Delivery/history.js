...
import { getAllCarsAPI, getAllDriversAPI, getHistoryDelivery, getListAntrianDeliveriAPI, selectDriversAPI } from "../../api/Delivery";
...
const HistoryDelivery = () => {
    ...
    const loadData = async () => {
        try {
            setLoading(true)
            let resHistory = await getHistoryDelivery();
            resHistory = resHistory.data;
            ...
            setMaster({
                ...master,
                today: resHistory.today,
                yesterday: resHistory.yesterday,
                driver: resDriver,
                car: resCar
            });
            setLoading(false);
    ...
    return (
        <ScrollView>
            <View style={classes.constainer}>
                <Text style={classes.textHeader}>Hari ini</Text>
                {
                    loading ?
                        <ActivityIndicator />
                        : null
                ...
                {
                    master.today.length > 0 ?
                        master.today.map((items, index) => {
                            return (
                                <View key={items.id} style={classes.tmpList}>
                                    <View style={[classes.card, classes.shadowProp]}>
                                        {isScheduledTimeValid(items.jadwal) && items.status === 1 ?
                                        ...
                                        <Text style={items.status === 1 ? classes.textStatus : items.status === 2 ? classes.textStatusSend : classes.textStatusArrived}>{items.status === 1 ? "Dalam Antrian" : items.status === 2 ? "Sedang Dikirim" : "Barang Diterima"}</Text>
                                    </View>
                ...
                <Text style={classes.textTitleYesterday}>Kemarin</Text>
                {
                    master.yesterday.length > 0 ?
                        master.yesterday.map((items, index) => {
                            return (
                                <View key={items.id} style={classes.tmpList}>
                                    <View style={[classes.card, classes.shadowProp]}>
                                        {isScheduledTimeValid(items.jadwal) && items.status === 1 ?
